"""Pull SHARADAR/SF1 dimension=ART for the Sharadar-native NYSE-only universe (1,168 names
already ingested into us_data_pit_full via nx_extend_nyse_sharadar.py). Mechanically
identical to ext10 (SP500) / nq6 (NASDAQ) -- same ART (As-Reported TTM) dimension, same
datekey (filing-date) indexing, same columns, same resumable per-batch checkpointing.

Ticker universe = sep_nyse_panel.h5's own ticker index, not nyse.txt (which drops one
short-history name at store-write time) -- the fundamentals pull should match the price
panel that will be used at merge time (nx_merge_causal_nyse.py).

WHY ART not MR: AR excludes restatements, indexed to the actual SEC filing date (datekey) =
the genuine point-in-time view. MR would leak future restatements. (Same reason as SP500/NASDAQ.)
"""
...
CHUNK = 100
CKPT = SH / "_sf1_nyse_batches"
CACHE = SH / "sf1_nyse_raw.h5"


def main():
    panel = pd.read_hdf(SH / "sep_nyse_panel.h5")
    tick = sorted(panel.index.get_level_values("ticker").unique())
    print(f"pulling SF1 dimension={DIM} for {len(tick)} NYSE-only tickers", flush=True)
    ...
    print(f"  ACTUAL SF1 coverage: {n_data} of {n_target} target NYSE-only names "
