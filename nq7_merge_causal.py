"""Causally merge SF1-ART fundamentals onto daily SEP prices for the NASDAQ-only universe.
Mechanically identical to ext11 (SP500): merge_asof(direction='backward') on datekey (filing
date), same 7 factors, same perturbation-based look-ahead check. Only inputs/outputs differ:
  prices  <- sep_nasdaq_panel.h5   fundamentals <- sf1_nasdaq_raw.h5
  output  -> fundamentals_nasdaq_daily.h5

Also reports the ACTUAL usable-coverage count: NASDAQ-only names that end up with a non-null
factor on a reasonable span (the number that matters for whether a factor test is feasible).
"""
...
START = "1999-01-01"
RAW = SH / "sf1_nasdaq_raw.h5"
PANEL = SH / "sep_nasdaq_panel.h5"
OUT = SH / "fundamentals_nasdaq_daily.h5"
...
def main():
    ...
    # restrict to the NASDAQ-only target list (panel also holds SP500-overlap names)
    target = set(pd.read_csv(SH / "nasdaq_only_tickers.csv")["ticker"].astype(str))
    close = close[[c for c in close.columns if c in target]]
    px_long = close.stack().rename("close").reset_index()
    px_long.columns = ["datetime", "ticker", "close"]
    print(f"price cells (NASDAQ-only priced (ticker,date) since {START}): {len(px_long):,}, "
    ...
    print(f"\n=== USABLE COVERAGE (NASDAQ-only names with a non-null factor) ===")
    for c in fac_cols:
        s = out[c]
    ...
    # ================= LOOK-AHEAD CHECK (same as ext11) =================
